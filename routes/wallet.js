...
router
  ...
  .get('/symbols', function (req, res) {
    global.db.all('SELECT c.id, c.name, c.symbol FROM cryptocurrencies c where c.id not in (SELECT id_cryptocurrency FROM wallets_cryptocurrencies where id_user = ?)', [req.id],
      (error, data) => {
        if (error) {
          console.debug(error)
          res.status(500).send('Internal Server Error'); return
